 <?php
	function db_connect(){
		$conn = mysqli_connect("localhost", "root", "", "bookstores1");
		 if(!$conn){
	...
	function getCustomerId($name,$username,$email,$ph, $address, $city, $zip_code,$country,$gn,$pass){
		$conn = db_connect();
	...
	function setCustomerId($name,$username,$email,$ph, $address, $city, $zip_code,$country,$gn,$pass){
		$conn = db_connect();
		$query = "INSERT INTO customers VALUES 
			('', '" . $name . "','" . $username . "','" . $email . "','" . $ph . "', '" . $address . "', '" . $city . "', '" . $zip_code . "', '" . $country . "','" . $gn . "','" . $pass . "')";

		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "insert false ! sorry" . mysqli_error($conn);
			exit;
		}
